...
def randomize_round(gg, round_num, event_name):
    selected_strategy = random.choice(gg["strategies"])
    selected_deployment = random.choice(gg["deployments"])
    selected_schemes_ids = random.sample(range(0, 13), 5)
    print(selected_schemes_ids)
    # selected_schemes_names = random.sample(gg["schemes"], 5)
    selected_schemes_names = [gg["schemes"][i] for i in selected_schemes_ids]
    return {
        "Name": f"{event_name} #{round_num + 1}",
        "Strategy": selected_strategy,
        "Deployment": selected_deployment,
        "Schemes": selected_schemes_names,
        "Schemes_ids": selected_schemes_ids,
    }
...
def main():
    ...
    rounds = [randomize_round(gg, n, params["event_name"]) for n in range(params["rounds"])]
